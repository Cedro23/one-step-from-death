...
func physics_update(_delta: float) -> void:
	...
	if character.global_position.distance_to(player.global_position) <= character.lunge_detection_distance:
		character.state_machine.transition_to("LungeState")
	elif character.global_position.distance_to(player.global_position) > character.player_escape_range:
		character.state_machine.transition_to("IdleState")
